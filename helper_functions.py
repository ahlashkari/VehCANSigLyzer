...
def preprocessHCRLDataset(df):
    '''
    Takes a dataframe formated as in the HCRL Attack & Defense dataset
    Returns a new dataframe with the log formatted into columns as in can-ml
    '''
    ...
    df['data_field'] = df['data_field'].str.replace(' ', '')                # Remove spaces in data field

    ## Labelling ##
    df['attack'] = df['Class'].apply(lambda x: 0 if x == 'Normal' else 1)   # Get binary labels
    ...
    if 'SubClass' in df.columns: 
        df['attack_class'] = df['SubClass'].apply(lambda x: labels[x])
    else: 
        df['attack_class'] = df['attack']

    df['vehicle_state'] = 0                 # 0 = stationary, 1 = driving
    ## Labelling ##

    df = df[['timestamp', 'arbitration_id', 'data_field', 'attack', 'attack_class', 'vehicle_state']]
...
def getTimeIntervals(df):
    ...
    # Rearrange columns
    df = df[['time_interval', 'aid_time_interval', 'arbitration_id', 'data_field', 'attack', 'attack_class', 'vehicle_state']]          
...
def deserializeCANDataFrame(df, db, attack=False, signals=None):
    ...
    try:
        ...
        df = pd.concat([df[[col for col in df.columns if col not in ['attack', 'attack_class', 'vehicle_state']]], dfsig_norm, df[['attack', 'attack_class', 'vehicle_state']]], axis=1)
